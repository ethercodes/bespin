...
// From MDC: https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/array/foreach
...
// From M$ - Emulate legacy getter/setter API using ES5 APIs
...
/*
	WebWorker shim using XHR and setTimeout.
	Modified from http://code.google.com/p/ie-web-worker/
*/
...
(function()
{
	...
	function xhrSync( url )
	{
		...
		if ( http.readyState == 4 )
		{
			switch ( http.status )
			{
				case 404:
				case 500:
					throw 'Fails to load script: ' + url;
				default:
	...
	if(!window.Worker)
	{
		window.Worker = function ( scriptFile )
		{
			var self = this ;
			// All scripts of this worker share the same scope.
			var scope = { window:undefined, document: undefined,bespin: undefined, tiki:undefined };
			var timer = null ;
			var msg_queue = [] ;
			var basehref_stack = [];
			var onmessage ;
			...
			// Load sub script with path relative of the current script synchronously via XHR,
			// evaluate it in the the worker scope.
			function importScripts(src)
			{
				// Peak the base href for this script.
				var base = basehref_stack[ basehref_stack.length - 1 ];
				src = base ? base + '/' + src : src;
				basehref_stack.push( src.replace( /\/[^\/]*$/, '' ) );
				with ( scope ) { eval( xhrSync( src ) ); }
				basehref_stack.pop();
				return true;
			}
			...
			var __iterate = function()
			{
				...
				timer = setTimeout(__onIterate,1);
				return true ;
			} ;

			function __onIterate()
			{
				try
				{
